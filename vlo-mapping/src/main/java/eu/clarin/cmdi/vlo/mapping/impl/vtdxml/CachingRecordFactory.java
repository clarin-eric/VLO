...
public class CachingRecordFactory implements RecordFactory {
    ...
    private final LoadingCache<File, Record> recordCache;

    public CachingRecordFactory(RecordReader recordReader) {
        recordCache = CacheBuilder.newBuilder().build(new CacheLoader<File, Record>() {
            @Override
            public Record load(File f) throws Exception {
