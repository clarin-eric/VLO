...
public class RasaResourceAvailabilityStatusCheckerTest {
    ...
    public void setUp() {
        checkedLinkResource = new CheckedLinkResource() {
            ...
            public Map<URI, CheckedLink> get(Collection<URI> uri, Optional<CheckedLinkFilter> filter) {
                return ImmutableMap.<URI, CheckedLink>builder()
            ...
            @Override
            public Stream<CheckedLink> getHistory(URI uri, CheckedLinkResource.Order order, Optional<CheckedLinkFilter> filter) {
                return null; //not needed for test
            }
        };
    ...
    public static AbstractMap.SimpleImmutableEntry<URI, CheckedLink> createResponseMapEntry(String url, int status) {
        final CheckedLink checkedLink = new CheckedLink();
        checkedLink.setUrl(url);
        checkedLink.setStatus(status);
        return new AbstractMap.SimpleImmutableEntry<>(URI.create(url), checkedLink);
    }
    ...
    public void testGetLinkStatusForRefs() {
        ...
        Map<URI, CheckedLink> result = instance.getLinkStatusForRefs(hrefs);
        assertEquals(2, result.size());
        assertNotNull(result.get(URI.create("http://uri1")));
        assertEquals("http://uri1", result.get(URI.create("http://uri1")).getUrl());
        assertEquals(200, result.get(URI.create("http://uri1")).getStatus());
        assertNotNull(result.get(URI.create("http://uri2")));
        assertEquals("http://uri2", result.get(URI.create("http://uri2")).getUrl());
        assertEquals(404, result.get(URI.create("http://uri2")).getStatus());
    }
