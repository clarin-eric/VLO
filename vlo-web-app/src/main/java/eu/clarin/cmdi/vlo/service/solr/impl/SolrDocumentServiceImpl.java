...
public class SolrDocumentServiceImpl implements SolrDocumentService {
    ...
    @Override
    public SolrDocumentExpansionPair getDocumentWithExpansion(String docId, String collapseField, QueryFacetsSelection selection, int expansionFirst, int expansionCount) {
        final SolrDocument doc = getDocument(docId);
        if (doc == null) {
            return null;
        } else {
            final String signature = doc.getFieldValue(collapseField).toString();
            final SolrQuery duplicatesQuery = queryFactory.createDuplicateDocumentsQuery(docId, collapseField, signature, selection, expansionFirst, expansionCount);
    ...
    public long getTotalDocumentCount(QueryFacetsSelection selection) {
        final SolrQuery query = queryFactory.createExpandedDocumentQuery(selection, 0, 0);
        return searchResultsDao.getDocuments(query).getNumFound();
    }
