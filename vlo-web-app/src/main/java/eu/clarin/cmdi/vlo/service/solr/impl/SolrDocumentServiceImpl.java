...
public class SolrDocumentServiceImpl implements SolrDocumentService {
    ...
    @Override
    public SolrDocumentExpansionPair getDocumentWithExpansion(String docId, String collapseField, int expansionFirst, int expansionCount) {
        final SolrDocument doc = getDocument(docId);
        if (doc == null) {
            return null;
        } else {
            final String signature = doc.getFieldValue(collapseField).toString();
            final SolrQuery duplicatesQuery = queryFactory.createDuplicateDocumentsQuery(docId, collapseField, signature, expansionFirst, expansionCount);
