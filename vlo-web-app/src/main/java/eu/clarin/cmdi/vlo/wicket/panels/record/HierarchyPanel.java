...
public class HierarchyPanel extends GenericPanel<SolrDocument> {
            ...
            protected void populateItem(final Item<SolrDocument> item) {
                ...
                final IndicatingAjaxFallbackLink upLink = new IndicatingAjaxFallbackLink("up") {
                    ...
                    public void onClick(AjaxRequestTarget target) {
                        logger.debug("Expand up: {}", item.getModelObject().get(fieldNameService.getFieldName(KEY.FIELD_ID)));
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug("Expand {} -> {}", PARENTS_ROOT_PATH, item.getModelObject().getFieldValue(fieldNameService.getFieldName(KEY.FIELD_NAME)));
                        }
                    ...
                    protected void updateCurrentExpansion() {
                        ...
                        for (ModelWrapper<SolrDocument> item : currentExpansion) {
                            ...
                            final String newPath = item.getPath().replaceFirst(PARENTS_ROOT_PATH, PARENTS_ROOT_PATH + "/" + item.getModelObject().getFieldValue(fieldNameService.getFieldName(KEY.FIELD_ID)));
                            tree.collapse(item);
                            ...
                            return new NamedRecordPageLink(id, node.getObject().getModel(), RecordPage.DETAILS_SECTION)
                                    .add(new AttributeAppender("class", new AbstractReadOnlyModel<String>() {
                                        ...
                                        public String getObject() {
                                            final boolean isCurrent = node.getObject().getModelObject().getFieldValue(fieldNameService.getFieldName(KEY.FIELD_ID))
                                                    .equals(pageDocumentModel.getObject().getFieldValue(fieldNameService.getFieldName(KEY.FIELD_ID)));
    ...
    private class HierarchyTreeProvider extends SortableTreeProvider<ModelWrapper<SolrDocument>, Object> {
        ...
        public Iterator<? extends ModelWrapper<SolrDocument>> getRoots() {
            if (logger.isDebugEnabled()) {
                logger.debug("Root: {} -> {}", PARENTS_ROOT_PATH, HierarchyPanel.this.getModel().getObject().getFieldValue(fieldNameService.getFieldName(KEY.FIELD_NAME)));
            }
        ...
        public boolean hasChildren(ModelWrapper<SolrDocument> node) {
            if (node.isLimit()) {
                ...
                final Object partCount = node.getModelObject().getFieldValue(fieldNameService.getFieldName(KEY.FIELD_HAS_PART_COUNT));
        ...
        public Iterator<? extends ModelWrapper<SolrDocument>> getChildren(final ModelWrapper<SolrDocument> node) {
            if (node.isLimit()) {
                ...
                final Collection<Object> parts = node.getModelObject().getFieldValues(fieldNameService.getFieldName(KEY.FIELD_HAS_PART));
    ...
    private class ParentsDataProvider implements IDataProvider<SolrDocument> {
        ...
        public Iterator<? extends SolrDocument> iterator(long first, long count) {
            final Collection<Object> parentIds = HierarchyPanel.this.getModelObject().getFieldValues(fieldNameService.getFieldName(KEY.FIELD_IS_PART_OF));
            if (parentIds == null) {
        ...
        public long size() {
            if (size == null) {
                final Collection<Object> fieldValues = HierarchyPanel.this.getModelObject().getFieldValues(fieldNameService.getFieldName(KEY.FIELD_IS_PART_OF));
                if (fieldValues == null) {
                    // no parent field
                    size = 0L;
                } else {
